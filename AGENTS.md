...
- Test locations:
    ...
    - `crates/honk-outbound/src/proxy/hysteria2/tests.rs`, `src/urltest/tests/http2.rs`, and inline `#[cfg(test)]` in `proxy/*`, `urltest.rs`, `bootstrap.rs` — wire vectors and test-only loopback/in-process QUIC interop (`quic::testutil`). Production transport: outbound **UDP transport invariant**.

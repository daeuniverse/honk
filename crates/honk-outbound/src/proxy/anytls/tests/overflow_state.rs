...
fn overflow_terminal_events_cap_per_stream() {
    ...
    assert_eq!(overflow.usage().frames, TEST_OVERFLOW_BURST_FRAMES);
    assert_eq!(overflow.stream_usage(1).frames, TEST_OVERFLOW_BURST_FRAMES);
}
...
async fn overflow_admit_below_hard_cap_never_kills() {
    ...
    assert_eq!(overflow.stream_usage(1).bytes, TEST_STREAM_BURST_BYTES + 1);

    let mut session_burst = OverflowState::default();
    for _ in 0..TEST_OVERFLOW_BURST_FRAMES {
        session_burst.push_back(1, StreamEvent::Data(InboundPayload::for_test(vec![1])));
    }
    tokio::time::advance(OVERFLOW_STALL_GRACE * 4).await;
    assert!(matches!(
        session_burst.admit(2, StreamEvent::Data(InboundPayload::for_test(vec![2])),),
        OverflowAction::Parked
    ));
    assert_eq!(session_burst.usage().frames, TEST_OVERFLOW_BURST_FRAMES + 1);
}

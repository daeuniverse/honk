...
async fn main() -> anyhow::Result<()> {
    ...
    let mut config_node = node.clone();
    let tls = config_node
        .tls_mut()
        .ok_or_else(|| anyhow::anyhow!("QUIC node has no TLS config"))?;
    tls.sni = Some("inner.test".into());
    tls.skip_cert_verify = true;
    let config =
        honk_outbound::quic::client_config(&config_node, &[b"h3"], Default::default()).await?;

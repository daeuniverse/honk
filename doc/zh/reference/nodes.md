...
**兼容性变更：** TOML、YAML 与 JSON 继续使用旧的扁平节点键，但加载现改为严格模式。与所选 `protocol` 不匹配的非默认字段会导致解析失败，不再被静默忽略；例如，带有 `tls: true` 的 `ss` 节点会被拒绝。升级前应清理模板或生成器遗留的无关字段。Honk 序列化器只会为协议不适用的字段输出可接受的默认值，因此自身输出仍可安全 round-trip。启用 `store_subscribe` 时，原始订阅正文仅在解析成功后持久化；被拒绝的刷新不会覆盖上一份有效正文。

...
Score 始终随程序编译，必须以 `policy: score` 显式选择；省略 `policy` 时仍默认使用 Selector。Score 没有运行时调节项，也不会改变其他策略的行为。

Score 先用普通健康过滤排除死亡候选，再作出权威的单成员选择。健康状态使用代理服务器可达的 IPv4/IPv6 地址族，与评分使用的业务目标地址族相互独立；因此 IPv4 代理服务器仍可承载 IPv6 目标。在剩余成员中，Score 首先按带 Beta 先验的可靠性下置信估计排名，再考虑 setup/首响应延迟和有界吞吐贡献。只有未训练候选会进入确定性冷探索；已训练候选不会因 attempt 数较少而获得路由 bonus。最终平局依次按声明顺序和稳定节点身份解决，因此选择既不会竞速多个成员，也不会使死亡节点重新入选。

冷探索对不超过 4 个成员的组会覆盖每个候选。更大的组只采样 `ceil(sqrt(n)) + 1` 个候选（不超过组大小），并周期性重新检查尝试次数最少的非当前成员；周期检查每进行 `2n` 次 Score 选择触发一次，间隔限制在 16–64 次选择。cadence 的作用域为 `(group, TCP/UDP, 目标 IPv4/IPv6 地址族或 none)`，绝不包含精确目标、节点、代理健康地址族、地址或端口。这样小组能完整覆盖，大组也不会把路由预算耗在一次性探测上。

所有历史计数、加权和与权重都采用固定 30 分钟半衰期的指数衰减。新鲜失败证据会绕过当前节点的 `0.01` utility 切换余量；全局、目标地址族和精确目标的失败 envelope 取最大值而非相加，只有这一有效衰减值低于一个样本的 `0.01` 后普通余量才会重新生效。setup 和首响应延迟取衰减加权均值。吞吐证据仅接纳终态成功、收发双向均非零、持续至少 1 秒，且主导方向传输至少 64 KiB 的交换；合格窗口以主导方向字节数除以总持续时间汇总，避免双计请求与响应，也不让短小突发影响排名。这些半衰期和门槛是固定实现常量，不可配置。
...
每次已授权的多候选 Apply 都按固定优先级只记录一个原因：初始探索（`coldExplore`）、周期探索（`periodicExplore`）、保持现任（`incumbentHeld`）、新鲜失败绕过（`freshFailureBypass`）、可靠性胜出（`reliabilityWinner`）或性能胜出（`performanceWinner`）。`deadFiltered` 独立记录被活性过滤移除的唯一叶候选，可以与一个原因同时增加。Peek 与展示/API 读取保持中性。经鉴权的 `/stats.score.groups[]` 只导出这些按组汇总的 TCP/UDP 计数和组名，不导出 cell、节点、目标、cadence 或 manager authority 数据。

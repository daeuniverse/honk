...
Comparison first uses qualified joint response-block intersections; original pairwise adverse evidence still vetoes support. Pair-focused validation uses one target-bound control/challenger run per group/network/family, limited to 45 seconds, 16 matching offers and eight optional reservation attempts. Already-qualified but misaligned cohorts instead distribute funded observations through the existing recency sampler, without waiting for another four-credit bank; ordinary forwarding provides controls. Both paths preserve business currency, in-flight bounds and live or still-needed unrelated-target focus, and never replay requests. Finished/dropped staged work and completed/invalid bound pairs release focus for other offered targets. Readonly coverage and blocker fields explain remaining gaps without exposing target identities.

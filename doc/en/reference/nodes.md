...
| `plugin` / `plugin_opts` | string? | null | Parsed SIP002 plugin metadata. Non-empty values are rejected for static nodes during config validation and skipped during subscription import because proxy plugins are unsupported |

...
Publication makes the replacement immediately available to new leases and moves the old runtime to draining. The old runtime waits for leases, closes prefetch and DNS transports, then drains its pinned outbound session pools. After the 30-second lease-drain deadline, runtime-backed service queries are cancelled before transport closure; this includes raw queries, controller outcomes, and both application name-resolution paths. At most four retired runtimes remain retained; cap eviction and provider shutdown use the same query cancellation. Ingress adapters return `SERVFAIL` for cancelled resolutions and release their process-wide admission permits after reply I/O, so stuck old-generation queries cannot indefinitely occupy the shared UDP slow-path budget (at most 256). Provider-owned retirement supervisors are reaped and joined during shutdown.

...
| `score/comparison/`, `score/verification.rs` | Bounded observation storage, paired/joint response evidence, validation questions and readonly pairwise relations; `score/tests/` splits scenarios by ownership |
...
The default objective is response quality with observed business-availability protection; directional goodput participates in ordinary promotion only when comparable evidence exists. Forwarding remains immediate. `provisional` and `observedUsable` describe the selected path's business evidence. Readonly `challengers` separately report fresh pairwise response relations between the selected member and evaluated challengers; there is no group-wide comparison claim. A 10% metric tolerance represents practical equivalence, not an error probability or a proof of global optimality.

A relation does not use the ordinary switching margin or its completion maturity. It compares actual response values with an inclusive symmetric 10% band (`high - low <= 0.1 × low`), including zero and sub-millisecond responses, allowing only floating-point roundoff at the boundary: inside the band the pair is `equivalent`, otherwise the lower response is faster. Relations read the selection's original pairs, the same pairs ordinary promotion reads, and only their fresh qualified response metric; reliability, availability, directional goodput and holding stay with ordinary selection. Each relation names its basis and expires with that response metric.

Before requesting response-alignment work, Score intersects fresh shared blocks across covered eligible challengers only and requires four distinct reporters per side within that intersection. Selection depends on identity/time/qualification, not measured values. Qualified joint support can align different pair histories immediately. Optional members neither narrow this intersection nor request alignment work. Original pairs remain authoritative for promotion and readonly relations. Insufficient covered joint support requests funded response validation. No extra probes, copied requests or arbitrary-load completion guarantee are introduced.
...
The same evaluator identifies missing availability and response evidence, chooses the next budgeted real-flow validation, and produces readonly API summaries. An exact-target view never labels common-target or probe evidence as `targetResponse`. Successful HEAD/QUIC probes can support only their current configured measurement cohort, never target usability or bulk capacity. `nextBusinessFlow` names future real-work demand, not a reservation or dispatched I/O; `question` and `waitReason` distinguish missing evidence from budget, in-flight, comparable-traffic and backoff waits. Missing transfer evidence creates no question or work: directional goodput waits for real offered load, and no background bandwidth scan or copied user request is created. Aggregate reads inspect retained IPv4/IPv6 budget waits without creating or spending a scope.

Business comparison support retains its 60-second bound; configured-probe comparison follows the cadence-aware retention and latest-support bounds above. Failed business outcomes and committed reload revoke recent usability without erasing long-term reliability. API reads derive current validity without advancing clocks, trials or counters. Authorized Apply records provisional, usable and validation selections in existing bounded history. See [API semantics](../reference/api.md#score-verification).
...
Only authorized Apply initializes or refreshes participants and admits currently qualified ranked members. Reranking is at most every five minutes; growth happens at refresh and shrinkage requires k to fall by at least two. Filtered/retry absence alone does not remove retained identities or admission. Accepted business feedback also latches qualification for already-ranked members under the state lock and existing validity/incarnation fences, including terminal outcomes and live recovery. Admission survives a lapse until ranked removal or reload. Readonly/Peek uses committed participants without reranking or admitting members; before initialization only cold configured-probe bootstrap may use a temporary bounded projection.

Coverage includes the selection and admitted ranked members when the current baseline has any qualified member; otherwise every ranked member is covered. This cold gate is current, not a historical once-ever epoch. Only covered members' response alignment requests response validation. While that gate is active, ranked members awaiting first qualification and the rotation member can receive optional work without joining joint alignment. A lapse after admission keeps coverage rather than shrinking it.

Unevaluated members are never compared and receive no new optional reservations; a run loses its witness when its challenger leaves. Previously reserved non-run work may still begin under its original token, identity and TTL checks; leaving the set neither refunds begun work nor creates free work. Probe baselines remain in stats, but comparison-cell admission normally follows the evaluation set, with exceptions for up to four recent Apply winners and initial/no-initialized-set collection. Ordinary first choice, escape and recovery still consider all members. Membership checks are bounded by k; reranking costs O(n log n), while rotation selects the next identity in one allocation-free pass. Time-based evaluation rotation is separate from begin-driven validation recency.

A recent failure exclusion removes an ineligible member from pending validation until its failure is older than the performance evidence age. Response alignment also skips reliability-dominated members: the member is outside the eligibility band, and it and the selection both have four effective completions while its observed reliability is lower. Neither case measures the member as equivalent or nonregressing, and decaying layer weights can make it eligible again earlier. Response degradation, which reopens comparison freshness, compares a setup or response sample only with the same exact target's history; aggregate cells mix targets of unrelated latency, so aggregate comparisons reopen on carrier pressure rather than cross-target differences.

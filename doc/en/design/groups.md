...
| `score/ranking.rs`, `score/budget.rs` | Ordinary ranking; retained business currency and in-flight accounting |
| `score/evidence.rs`, `score/feedback.rs` | Scoped outcome/provenance rules and exact-once observation ownership |
| `score/comparison/`, `score/verification.rs` | Bounded observation storage, paired response evidence, validation questions, the single optional-work dispatch walk and readonly pairwise relations; `score/tests/` splits scenarios by ownership |
...
An ordinary pending attempt captured before a Score authority change may proceed unscored only when it owns no optional token. Its guard preserves the original business identity without publishing current evidence or currency. Invalid optional work still refuses; it never becomes a free challenger dial. Physical runtime admission, routing and typed refusal semantics remain unchanged.
...
Reservations are spent only when node-specific work begins. Releasing or invalidating the last unbegun reservation refunds it; begun cancellation never refunds. Reload retains started in-flight tracking until settlement or its existing expiry. UI redirects, DNS retries and TCP replacements retain the original business identity and cannot mint trials or relabel an existing trial as free recovery. There is one optional-work path: an authorized targeted rank walks the evaluated challengers in dispatch order and reserves one credit for the first member the ledger admits; a budget refusal ends the walk, and an in-flight refusal tries the next member. Each member has at most one unanswered trial for its open question on a target; qualification counts across targets. Missing traffic stays inconclusive. Real failures retain 5-minute-to-6-hour backoff unless genuine scoped RX opens recovery; trials never acquire committed-incumbent protection.

A member's open question is recovery after a failure streak; availability without recent usability; a pair when ordinary selection could compare it but the selection's original pair with it lacks a fresh, non-partial business response of this scope (exact target, or common targets for an aggregate read) newer than the selection's latest degradation; otherwise qualification. Configured probes never settle a pair question. Dispatch order puts untrained members first, then the most progress toward the open question (for a pair, the challenger reporters already sharing live blocks with the reference), then the least recently selected member; the configured-probe hint breaks ties among never-selected members. Recency advances only when attributed work is admitted at begin, so unbegun, refused or unscored plans leave the order unchanged. Except for recovery, a member is dispatched only while its completions plus its unfinished work stay below the selection's completions: first choice follows evidence, so training challengers first would displace the probe-preferred selection, and cold exploration therefore starts once the selection has a completion. The readonly question, action and wait come from the first member of the same order; the wait adds only the ledger's current budget or in-flight refusal. Stored node provenance names failure time separately from invalidation; borrowed cell stamps share current-parent and effective-fence checks without adding a cache.
...
Pairs need no joint alignment: each challenger's original pair with the selection is judged on its own shared blocks, and different challengers' histories create no validation demand against each other. No extra probes, copied requests or arbitrary-load completion guarantee are introduced.
...
Only authorized Apply initializes or refreshes participants. Reranking is at most every five minutes; growth happens at refresh and shrinkage requires k to fall by at least two. Filtered/retry absence alone does not remove retained identities. Readonly/Peek uses committed participants without reranking; before initialization, readonly reads use a temporary bounded projection.

Unevaluated members are never compared and receive no new optional reservations. Previously reserved work may still begin under its original token, identity and TTL checks; leaving the set neither refunds begun work nor creates free work. Probe baselines remain in stats, but comparison-cell admission normally follows the evaluation set, with exceptions for up to four recent Apply winners and initial/no-initialized-set collection. Ordinary first choice, escape and recovery still consider all members. Membership checks are bounded by k; reranking costs O(n log n), while rotation selects the next identity in one allocation-free pass. Time-based evaluation rotation is separate from begin-driven validation recency.

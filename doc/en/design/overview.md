...
- **Score isolation and reasons:** Health filtering and business-target family remain independent. First-choice utility stays heuristic; healthy promotion compares every eligible challenger in a bounded, demand-sized evaluation set with a fixed incumbent using shared target/time evidence, not unrelated aggregate/setup averages. Optional trials share a retained group/network/family budget funded by original business starts, never elapsed time; trials do not become committed incumbents. Real-flow streaks retain backoff and three-failure exclusion. Exact and aggregate evidence use separate 4,096-entry LRUs; comparison storage is capped at 256 cells and 1 MiB of logical allocation, not process RSS. Overlapping counts are not added. Reload preserves admitted business outcomes while revoking recent confirmation and probe baselines. Public summaries and counters stay scope-limited and never expose raw scorer target keys; see [group design](./groups.md#score-scoring-and-lifecycle).
- **Demand-driven Score feedback:** Setup/response and bounded directional progress publish before terminal settlement without extra Beta completions. Traffic, configured health and warm-up sources have separate roles; arbitrary manual delay measurements never become business reliability or a configured baseline. Score TCP setup can recover once through a different permitted leaf under the original deadline, without new final edges, Selector escape or payload replay.
- **Conditional verification surface:** Score proxy objects publish aggregate provisional/usable state, comparison basis, evidence questions, wait reasons and remaining validity with the same readonly selected member. Active-challenger `localComparison` does not certify incomplete global coverage. `/stats` adds fixed group/network verification and budget/cost fields plus a nested-deduplicated root business count; observed trial failures are not causal extra failures. Neither surface exports raw target keys or a probability of optimality. See [API verification](../reference/api.md#score-verification).
